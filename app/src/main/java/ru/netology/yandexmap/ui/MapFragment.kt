...
import ru.netology.yandexmap.databinding.MapFragmentBinding
import ru.netology.yandexmap.databinding.PlaceBinding
import ru.netology.yandexmap.viewmodel.MapViewModel
...
class MapFragment : Fragment(R.layout.map_fragment) {
    ...
    override fun onCreateView(
        ...
        /** стартовая точка загрузки на карте*/

        mapView = binding.map
        //mapView?.map?.mapType = MapType.SATELLITE
        mapView?.map?.move(
            CameraPosition(Point(59.607557, 39.573477), 16.0f, 0f, 0f),
            Animation(Animation.Type.SMOOTH, 20F), null
        )
